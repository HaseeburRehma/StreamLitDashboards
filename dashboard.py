...
# Create for Segment
segment = st.sidebar.multiselect("Pick the Segment", df["Segment"].unique())
if not segment:
    df4 = df3.copy()
else:
   df4 = df3[df3["Segment"].isin(segment)]

# Filter the data based on Region, State, City, and Segment
if not region and not state and not city and not segment:
    filtered_df = df
elif not state and not city and not segment:
    filtered_df = df[df["Region"].isin(region)]
elif not region and not city and not segment:
    filtered_df = df[df["State"].isin(state)]
elif state and city and segment:
    filtered_df = df4[df4["State"].isin(state) & df4["City"].isin(city) & df4["Segment"].isin(segment)]
...
elif city and segment:
    filtered_df = df4[df4["City"].isin(city) & df4["Segment"].isin(segment)]
else:
   filtered_df = df4[df4["Region"].isin(region) & df4["State"].isin(state) & df4["City"].isin(city) & df4["Segment"].isin(segment)]
